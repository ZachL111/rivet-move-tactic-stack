...
`rivet-move-tactic-stack` is centered on chess and game engines work in Elixir. The technical target is: Build an Elixir toolkit that studies tactic behavior through framed sample traffic, with bounds and ordering tests and explicit failure cases.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 2.
